...
# 扩展 FK：带夹爪 + 力传感器末端延伸
# ------------------------------------------------------------
# 背景: Piper 的 link6(第6关节) 之后还有:
#   - 夹爪: joint7/8 (两指, prismatic 沿 z 伸出 0.13503 m)
#   - 力传感器: 额外 7.6 cm 固定在夹爪末端
# 真实接触白板的是"传感器末端", 不是 link6。
# 这些函数在保留原 fk() 的基础上新增, 不影响原解算。
# ============================================================

# 夹爪长度 (URDF joint7/8 平移量)
GRIPPER_LENGTH = 0.13503
# 力传感器额外长度
SENSOR_LENGTH = 0.076
# 总末端延伸 (link6 末端 -> 传感器末端)
TOOL_EXTENSION = GRIPPER_LENGTH + SENSOR_LENGTH  # 0.211 m


def fk_tool(q, joint_indices=(0,1,2,3,4,5), tool_offset: float = TOOL_EXTENSION):
    """FK 到工具末端 (link6 沿其 z 轴再延伸 tool_offset)。

    夹爪/传感器沿 link6 的 z 轴方向伸出 (URDF joint7 xyz=(0,0,0.13503))。
    返回 link6 位姿 T, 其 z 轴方向偏移 tool_offset 即传感器末端。
    """
    T = fk(q, joint_indices)
    # link6 的 z 轴单位向量 (旋转矩阵第 3 列)
    z_axis = T[:3, :3] @ np.array([0.0, 0.0, 1.0])
    T_tool = T.copy()
    T_tool[:3, 3] = T[:3, 3] + z_axis * tool_offset
    return T_tool


def tool_pose_from_state(state, joint_indices=(0,1,2,3,4,5), tool_offset: float = TOOL_EXTENSION):
    """从 state 求工具(传感器)末端位姿 (xyz, rpy)。姿态与 link6 相同, 位置前移 tool_offset。"""
    q = np.array(state, dtype=np.float64)[:6]
    T = fk_tool(q, joint_indices, tool_offset)
    return pose_to_xyz_rpy(T)


def pose_from_joints(q, joint_indices=(0,1,2,3,4,5), tool_offset: float = TOOL_EXTENSION):
    """直接传入 6 关节角, 求工具末端 (xyz, rpy)。"""
    return tool_pose_from_state(q, joint_indices, tool_offset)
