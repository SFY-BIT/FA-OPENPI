    ...
    *polaris_config.get_polaris_configs(),
    ...
    TrainConfig(
        ...
        model=pi0_force.Pi0ForceConfig(
            ...
            force_head_loss_weight=0.01,
            force_frame_spike_weight=2.0,
            # EEF-only: 数据集已转换为 EEF rot6d 坐标 (state = xyz+rot6d+grip+force,
            # action = relative delta xyz+rot6d+grip), loss 直接在 EEF 空间算。
            # rot6d 连续无奇点, 无万向锁问题; action 已是相对 delta (矩阵合成),
            # 不启用 DeltaActions (rot6d 不能线性减)。
            use_eef_loss=False,
            num_experts=4, num_top_k=1,
        ),
        new_module_lr_multiplier=5.0,
        data=LeRobotPiperDataConfig(
            repo_id="/data/group1/junjie008/datasets/total_2task_flexiv_eef_rot6d",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
    ...
    TrainConfig(
        ...
        model=pi0_force.Pi0ForceConfig(
            ...
            force_head_loss_weight=0.01,
            force_frame_spike_weight=2.0,
            # EEF rot6d 数据集: loss 直接在 EEF 空间算, 无需 FK 分支
            use_eef_loss=False,
            num_experts=4, num_top_k=1,
        ),
        new_module_lr_multiplier=5.0,
        data=LeRobotPiperDataConfig(
            repo_id="/mnt/hdd/sfy/datasets/total_2task_flexiv_eef_rot6d",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="perform the task",
            use_delta_joint_actions=False,
    ...
    #   与 *_remote 完全相同, 唯一差异: batch_size=16
    #   用于论文/文档展示统一 bs=16 的标准配置
    # ────────────────────────────────────────────────────────────────────────
    TrainConfig(
        name="pi05_force_total_task_eef_only_standard",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
    ...
    #   与 pi05_force_total_task_* 的区别: 仅去掉 LIMoE 和 force 头,
    #   其余完全一致 (数据集/bs/warmup/冷启动 pi05_base/40k 步)
    #   1) joint: total_2task_flexiv_ft60 (7 维关节 state)
    #   2) eef  : total_2task_flexiv_eef  (7 维 EEF state)
    # 用途: 验证 LIMoE/力觉对任务的贡献 (纯 pi05 baseline)
    # ────────────────────────────────────────────────────────────────────────
    TrainConfig(
        name="pi05_plain_total_task_joint",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
    ...
    TrainConfig(
        name="pi05_plain_total_task_eef",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/mnt/hdd/sfy/datasets/total_2task_flexiv_eef_rot6d_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="perform the task",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/data/group1/junjie008/datasets/total_2task_flexiv_eef_rot6d_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="perform the task",
            use_delta_joint_actions=False,
    ...
    # ═───────────────────────────────────────────────────────────────────────
    # total_task_peel 五线配置 (2026-08-22)
    #   数据集链: total_task_peel_flexiv → ft60 / ft60_noforce / eef_abs(_noforce)
    #   任务: "peel cucumber" (单任务, 41 ep / 21892 帧)
    #   五线: plain joint / force joint_only / plain EEF v2 / force EEF v2 / force dual
    #   每线 remote (远端训练) + local (本地归一化/训练) 两组, 仅 repo_id 不同。
    #   EEF 线一律 v2 (--absolute 数据集 + use_eef_delta_actions)。
    #   训练编排: 脚本1 双joint并行→双EEF并行; 脚本2 双EEF resume→dual bs16 串行。
    # ═───────────────────────────────────────────────────────────────────────
    TrainConfig(
        name="pi05_plain_total_task_peel_joint_remote",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/data/group1/junjie008/datasets/total_task_peel_ft60_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
    ...
    TrainConfig(
        name="pi05_plain_total_task_peel_joint_local",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/mnt/hdd/sfy/datasets/total_task_peel_ft60_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="peel cucumber",
            use_delta_joint_actions=True,
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_joint_only_remote",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_joint_only_local",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
    ...
    TrainConfig(
        name="pi05_plain_total_task_peel_eef_v2_remote",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/data/group1/junjie008/datasets/total_task_peel_eef_abs_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="peel cucumber",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_plain_total_task_peel_eef_v2_local",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        data=LeRobotPiperDataConfig(
            repo_id="/mnt/hdd/sfy/datasets/total_task_peel_eef_abs_noforce",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="peel cucumber",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_eef_v2_remote",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        new_module_lr_multiplier=5.0,
        data=LeRobotPiperDataConfig(
            repo_id="/data/group1/junjie008/datasets/total_task_peel_eef_abs",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="peel cucumber",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_eef_v2_local",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
        ...
        new_module_lr_multiplier=5.0,
        data=LeRobotPiperDataConfig(
            repo_id="/mnt/hdd/sfy/datasets/total_task_peel_eef_abs",
            observation_image_key="observation.image",
            observation_wrist_image_key="observation.wrist_image",
            default_prompt="peel cucumber",
            use_delta_joint_actions=False,
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_eef_joint_remote",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
    ...
    TrainConfig(
        name="pi05_force_total_task_peel_eef_joint_local",
        model=pi0_force.Pi0ForceConfig(
            pi05=True, action_horizon=30, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora",
            action_expert_variant="gemma_300m_lora",
