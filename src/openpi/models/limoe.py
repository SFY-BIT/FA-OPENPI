...
from openpi.models.flaxformer_minimal import routing
from openpi.models.flaxformer_minimal import scatter_utils
from openpi.models.flaxformer_minimal import dense
from openpi.models.flaxformer_minimal.types import Array
from openpi.models.flaxformer_minimal.types import DType
...
# Async routing metrics storage for wandb logging (host callback, low overhead).
_ROUTING_METRICS: dict[str, float] = {}


def _store_routing_metrics(
    auxiliary_loss: float,
    router_z_loss: float,
    fraction_tokens_left_behind: float,
    router_confidence: float,
    expert_usage: float,
    top1_fraction: np.ndarray,
):
    _ROUTING_METRICS["moe_aux_loss"] = float(auxiliary_loss)
    _ROUTING_METRICS["moe_router_z_loss"] = float(router_z_loss)
    _ROUTING_METRICS["moe_fraction_left_behind"] = float(fraction_tokens_left_behind)
    _ROUTING_METRICS["moe_router_confidence"] = float(router_confidence)
    _ROUTING_METRICS["moe_expert_usage"] = float(expert_usage)
    for idx, frac in enumerate(np.asarray(top1_fraction).reshape(-1)):
        _ROUTING_METRICS[f"moe_top1_frac_expert_{idx}"] = float(frac)
...
class MoeLayer(nn.Module):
    ...
    def _scatter_to_experts(self, token_inputs: Array, enable_dropout: bool,
        ...
        **kwargs: Optional keyword arguments to pass to experts.
        ...
        top1_expert = router_indices.dispatch_indices[..., 0, 0]
        top1_valid = jnp.logical_and(
            router_indices.dispatch_indices[..., 0, 0] < self.num_experts,
            router_indices.dispatch_indices[..., 0, 1] < expert_capacity,
        )
        top1_counts = jnp.bincount(
            jnp.clip(top1_expert, 0, self.num_experts - 1).reshape(-1),
            weights=top1_valid.astype(jnp.float32).reshape(-1),
    ...
    def _mask_and_dispatch_to_experts(self, token_inputs: Array,
        ...
        **kwargs: Optional keyword arguments to pass to experts.
        ...
        router_mask = self.router(
            ...
            expert_capacity,
            apply_jitter=enable_dropout)
        top1_counts = router_mask.dispatch_mask.sum(axis=(0, 2)).astype(jnp.float32)
    ...
    def _call_experts(self, inputs: Array, enable_dropout: bool,
                        **kwargs) -> Array:
        """Apply each expert to its routed tokens (single-device, no SPMD).

        Replaces the original flaxformer all-to-all SPMD communication
        pattern with a simple vmap over the expert parameter axis.
        This avoids the strict reshape constraints of the original which
        required num_groups % num_experts == 0.

        Args:
        inputs: <float>[num_groups, num_experts, expert_capacity, hidden_dim]
            inputs to be dispatched to experts. Each slice across the second
        ...
        **kwargs: Optional keyword arguments to pass to experts.
        ...
        # Flatten group+capacity into a single sequence for each expert:
        # [num_groups, num_experts, capacity, hidden] → [num_experts, num_groups * capacity, hidden]
        inputs = jnp.transpose(inputs, (1, 0, 2, 3))
        inputs = inputs.reshape(num_experts, num_groups * capacity, hidden_dim)

        # Apply each expert independently via vmap over the expert axis (0).
        ...
        outputs = flax_partitioning.vmap_with_axes(
            layer_fn,
            in_axes=(0,),  # 0 is the expert dimension
            out_axes=0,
        ...
        # Reshape back: [num_experts, num_groups * capacity, hidden]
        #            → [num_groups, num_experts, capacity, hidden]
    ...
    def _sow_expert_metrics(self, auxiliary_loss: float, router_z_loss: float,
        ...
        total_top1 = jnp.maximum(jnp.sum(top1_counts), 1.0)
        top1_fraction = top1_counts / total_top1
        jax.debug.callback(
            _store_routing_metrics,
            auxiliary_loss,
            router_z_loss,
...
def _num_groups(num_tokens: int, max_group_size: int, num_experts: int) -> int:
    ...
    - num_tokens % num_groups = 0 (group size evenly divides num_tokens).
    - num_groups % num_experts = 0 (for SPMD — relaxed if no valid n found).

    If no valid num_groups exists, falls back to num_groups = num_tokens
    (one token per group), which always works for single-device training.
    """
    min_num_groups = num_tokens // max_group_size
    min_num_groups = max(min_num_groups, 1)

    # First pass: try to satisfy both constraints (SPMD-compatible)
    def viable_strict(n):
        return num_tokens % n == 0 and n % num_experts == 0

    num_groups = max(min_num_groups, num_experts)
    while num_groups < num_tokens and not viable_strict(num_groups):
        num_groups += 1

    if viable_strict(num_groups):
        return num_groups

    # Second pass: relax num_experts constraint (single-device)
    def viable_relaxed(n):
        return num_tokens % n == 0

    num_groups = min_num_groups
    while num_groups < num_tokens and not viable_relaxed(num_groups):
        num_groups += 1

    if not viable_relaxed(num_groups):
        # Fallback: one token per group (always works)
