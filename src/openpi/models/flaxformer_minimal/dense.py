...
try:
    ...
    aqt_flax_layers = None  # type: ignore
    aqt_config = None  # type: ignore
    # Provide a dummy aqt namespace for type annotations
    from types import SimpleNamespace as _SN
    aqt = _SN(QuantOps=_SN(WeightParams=None, ActHParams=None), QuantType=_SN())  # type: ignore
    _HAS_AQT = False
...
from openpi.models.flaxformer_minimal import activation_partitioning
from openpi.models.flaxformer_minimal.types import Array
from openpi.models.flaxformer_minimal.types import DType
from openpi.models.flaxformer_minimal.types import Initializer
  ...
  def __call__(self,
    ...
    def dense(features, name, inputs, kernel_axis_names):
      if self.use_aqt:
        if not _HAS_AQT:
          raise ImportError(
              'AQT quantization is not available. Install aqt to use use_aqt=True.'
          )
